...
def esc(s):
    ...
    Raises rather than substituting '?': the base-14 fonts are not embedded, so
    any character outside WinAnsi (IPA symbols, arrows, emoji, Devanagari) would
    silently corrupt the page. Better to fail the build.
    """
    bad = sorted({c for c in s if not _encodable(c)})
    if bad:
        raise ValueError("characters outside WinAnsi: %s in %r"
                         % (" ".join("U+%04X %s" % (ord(c), c) for c in bad), s[:60]))
    out = []
    for ch in s:
        if ch in "()\\":
            out.append("\\" + ch)
...
ACCENT = (0.13, 0.21, 0.44)
ACCENT2 = (0.78, 0.41, 0.04)
TINT = (0.937, 0.945, 0.968)
TINT2 = (0.992, 0.957, 0.906)
RULE = (0.80, 0.83, 0.84)
...
class Book(Doc):
    ...
    def day_heading(self, num, title, focus):
        ...
        self.y -= 26
        for ln in wrap_runs(parse_runs(title, "B"), self.tw, 17.0):
            self.draw_line(self.ml, self.y, ln, 17.0, ACCENT)
            self.y -= 20
        self.y += 3
        self.space(2)
        self.para("*Focus:* " + focus, size=9.7, lead=13.4, color=MUTED)
        self.space(4)
    ...
    def h3(self, text, space_before=6, space_after=2):
    ...
    # -- call-outs ---------------------------------------------------------
    def box(self, label, blocks, fill=TINT, bar=ACCENT, size=9.5, lead=13.1):
    ...
    def scripts(self, items, label="Say it like this", lead=13.2):
        self.h3(label)
        size = 9.5
        for it in items:
